...
def is_approved(update: Update) -> bool:
    uid = get_user_id(update)
    return uid in APPROVED_USER_IDS or uid in OWNER_IDS


async def access_guard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    # Never allow groups/channels.
    ...
    # Approval required for everyone else.
...
async def request_access(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    uid = get_user_id(update)
    if uid in OWNER_IDS:
        await update.message.reply_text("👑 You are an owner.")
        return MENU
    ...
    # Send the request to EVERY configured owner ID.
    for owner_id in OWNER_IDS:
        try:
            keyboard = ReplyKeyboardMarkup(
                [[f"✅ Approve {uid}", f"❌ Reject {uid}"]],
            ...
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"Command fallback: `/approve {uid}`"
                ),
                parse_mode="Markdown",
...
async def approve_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # Also supports button callback-like text through normal messages.
    if uid is None and update.message and update.message.text:
        match = re.search(r"(?:Approve|approve)\\s+(\\d+)", update.message.text)
        if match:
    ...
    await update.message.reply_text(
        f"✅ User `{uid}` approved successfully.\\n"
        "They can now use the bot in private chat.",
        parse_mode="Markdown",
    )
    ...
    try:
        await context.bot.send_message(
            chat_id=uid,
            text="✅ *Access Approved!*\\n\\nOwner approved your access. Send /start to use the bot.",
            parse_mode="Markdown",
        )
    except Exception as e:
...
async def revoke_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if uid is None and update.message and update.message.text:
        match = re.search(r"(?:Reject|revoke|Revoke)\\s+(\\d+)", update.message.text)
        if match:
            uid = int(match.group(1))
...
async def owner_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        f"*Owner IDs:*\\n{owners_text}\\n\\n"
        "All these values are editable directly in the source file.",
        parse_mode="Markdown",
    )
    return MENU
...
async def approve_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    if not is_private_chat(update) or not is_owner(update):
        return ConversationHandler.END
    await update.message.reply_text(
        "Send the user's numeric Telegram ID like:\\n/approve 123456789"
    )
    return MENU
